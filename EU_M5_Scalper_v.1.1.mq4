...
extern double TakeProfit = 9.99;
extern double Stoploss = 333.33;
extern double PositionsGap = 33.33; //Distance Btwn Orders
extern bool TrailSL = true; //Trailing StopLoss
extern int TrailPips = 9; //Trailing Pips
extern int MagicNumber = 33333;
extern int Slippage = 3;
extern int SpreadOp = 33.0; //Max Spread Allowed
  ...
  if (Hedging == true) {
    ...
    if (true) {
      ...
      if (l_str2dbl_28 == ld_36) {
        ...
        ld_160 = 0;
        for (g_pos_424 = OrdersTotal() - 1; g_pos_424 >= 0; g_pos_424--) {
          TicketOrder = OrderSelect(g_pos_424, SELECT_BY_POS, MODE_TRADES);
          if (OrderSymbol() != Symbol() || OrderMagicNumber() != g_magic_248) continue;
          if (OrderSymbol() == Symbol() && OrderMagicNumber() == g_magic_248) {
            if (OrderType() == OP_BUY || OrderType() == OP_SELL) {
              g_price_296 += OrderOpenPrice() * OrderLots();
                ...
                if (OrderStopLoss() > 0.0)
                  if (OrderModifyCheck(OP_BUY, g_price_296, OrderStopLoss(), g_price_252)) { //Print(" Error 1 From C");
                    if (CheckStopLoss_Takeprofit(OP_BUY, OrderStopLoss(), g_price_252))
                      TicketOrder = OrderModify(OrderTicket(), g_price_296, OrderStopLoss(), g_price_252, 0, Yellow);
                  }
                else {
                  if (OrderType() == OP_SELL)
                    if (OrderModifyCheck(OrderTicket(), g_price_296, OrderOpenPrice() + Stoploss / OneOverPoint, g_price_252)) { //Print(" Error 1 From D");
                      if (CheckStopLoss_Takeprofit(OP_SELL, OrderOpenPrice() + Stoploss / OneOverPoint, g_price_252))
                        TicketOrder = OrderModify(OrderTicket(), g_price_296, OrderOpenPrice() + Stoploss / OneOverPoint, g_price_252, 0, Yellow);
                    }
                  else
                  if (OrderType() == OP_BUY)
