...
extern string EAName = "2022";
extern int MagicNumber_Hilo = 575899;
...
int start() {
   ...
   if ((CloseFriday == TRUE && DayOfWeek() == 5 && TimeCurrent() >= StrToTime(CloseFridayHour + ":00")) || (OpenMondey == TRUE && DayOfWeek() == 1 && TimeCurrent() <= StrToTime(OpenMondeyHour + ":00"))) return (0);
   ...
   if (Gi_424) {
      if (Gi_360 == TRUE) {
         for (G_pos_392 = OrdersTotal() - 1; G_pos_392 >= 0; G_pos_392--) {
            TicketOrder = OrderSelect(G_pos_392, SELECT_BY_POS, MODE_TRADES);
            if (OrderSymbol() != Symbol() || OrderMagicNumber() != MagicNumber_Hilo) continue;
            if (OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber_Hilo) TicketOrder = OrderModify(OrderTicket(), G_price_312, OrderStopLoss(), G_price_280, 0, Yellow);
            Gi_424 = FALSE;
         }
      }
   }
...
void f0_24() {
   for (int pos_0 = OrdersTotal() - 1; pos_0 >= 0; pos_0--) {
      ...
      if (OrderSymbol() == Symbol()) {
         if (OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber_Hilo) {
            if (OrderType() == OP_BUY) TicketOrder = OrderClose(OrderTicket(), OrderLots(), Bid, slip_Hilo, Blue);
            if (OrderType() == OP_SELL) TicketOrder = OrderClose(OrderTicket(), OrderLots(), Ask, slip_Hilo, Red);
         }
         Sleep(1000);
      }
   }
