...
enum Oper {
   Stand_by_Mode,
   Normal_Operation,
   Close_In_Profit_And_Stop,
   Close_Immediately_All_Orders
};
enum Pair {
   Chart_Pair,
   Inputted_Pairs
...
#define MagicSet      5758
//====================================================================================================================================================//
extern string EquityMinStopEAStr  = "||---------- Add by Fikri ----------||";
extern double EquityMinStopEA     = 9600;
extern double EquityMaxFloating   = 400;
...
extern double StepOpenNextOrders  = 50.00;//Step For Next Order (Value $/Lot)
...
extern CloseP TypeCloseInProfit   = Advanced_Mode;//Type Of Close In Profit Orders
extern double TargetCloseProfit   = 50.00;//Target Close In Profit (Value $/Lot)
...
extern double TargetCloseLoss     = 50.00;//Target Close In Loss (Value $/Lot)
extern bool   UsePercentageClose  = false;//Use Percentage Loss Close
extern double PercentageEquity    = 95.55;//Percentage Equity Close
...
int OnInit() {
   ...
   //---------------------------------------------------------------------
   //Comments orders 
   if (StringOrdersEA == "") CommentsEA = WindowExpertName();
   else CommentsEA = StringOrdersEA;
   //---------------------------------------------------------------------
   //Set chart pair
...
void OnTick() {
   ...
   //Add by Fikri
   StepOpenNextOrders = NormalizeDouble(GetADRs(PERIOD_D1, 20, 1) / 1 / 10, 2);
   TargetCloseProfit = NormalizeDouble(GetADRs(PERIOD_D1, 20, 1) / 1 / 10, 2);
   TargetCloseLoss = NormalizeDouble(GetADRs(PERIOD_D1, 20, 1) / 1 / 10, 2);
   
   //---------------------------------------------------------------------
   //Reset value
   CallMain = false;
   //---------------------------------------------------------------------
   if ((IsOptimization()) || (IsVisualMode()) || (IsTesting())) {
      MainFunction();
   }
   //---------------------------------------------------------------------
   //Warning message
   ...
   //Add by Fikri
   if(AccountEquity() < EquityMinStopEA) {
      ExpertRemove();
      RemoveAllOrders();
   }
   if((AccountBalance() - AccountEquity()) > EquityMaxFloating) {
      ExpertRemove();
      RemoveAllOrders();
   }
   ...
   if(FloatingMin > (AccountEquity() - AccountBalance())) {
      FloatingMin = AccountEquity() - AccountBalance();
   }
   
   if(FloatingMax < (AccountEquity() - AccountBalance())) {
      FloatingMax = AccountEquity() - AccountBalance();
   }
...
void MainFunction() {
   ...
   for (int cnt = 0; cnt < NumberPairsTrade; cnt++) {
      ...
      if (((IsOptimization()) || (IsTesting())) && (NewBar[cnt] == false)) {
         return;
      }
      //---------------------------------------------------------------------
      ...
      {
         if (TotalOrdersPlus[cnt] > 0)
            for (i = 0; i <= TotalOrdersPlus[cnt]; i++) MultiplierStepPlus[cnt] += i;
         if (TotalOrdersMinus[cnt] > 0)
            for (i = 0; i <= TotalOrdersMinus[cnt]; i++) MultiplierStepMinus[cnt] += i;
      }
      //---
      if (StepOrdersProgress == 2) //Exponential
      {
         if (TotalOrdersPlus[cnt] > 0)
            for (i = 0; i <= TotalOrdersPlus[cnt]; i++) MultiplierStepPlus[cnt] += MathMax(1, MathPow(2, i - 1));
         if (TotalOrdersMinus[cnt] > 0)
            for (i = 0; i <= TotalOrdersMinus[cnt]; i++) MultiplierStepMinus[cnt] += MathMax(1, MathPow(2, i - 1));
      }
      //---------------------------------------------------------------------
      //Levels
...
void CommentChart() {
   ...
   if (UseTimeWindow == true) {
      ...
      if (TimeToTrade == false)
         if (ObjectFind("Text5") == -1) DisplayText("Text5", " Remain To Open: " + StringSubstr(HopenStr, 0, Hdigits) + ":" + StringSubstr(MopenStr, 0, Mdigits) + ":" + StringSubstr(SopenStr, 0, Sdigits), SizeFontsOfInfo, "Arial Black", TextColor, PosX, PosY + 60);
      if (TimeToTrade == true)
         if (ObjectFind("Text5") == -1) DisplayText("Text5", " EXPERT TRADE", SizeFontsOfInfo, "Arial Black", TextColor, PosX, PosY + 60);
   }
   //---------------------------------------------------------------------
   ...
   for (i = 0; i < NumberPairsTrade; i++) {
      ...
      //---
      if (MaxProfit == -99999) ShowMaxProfit = 0;
      else ShowMaxProfit = MaxProfit;
      if (MinProfit == 99999) ShowMinProfit = 0;
      else ShowMinProfit = MinProfit;
      ...
      if (ShowPairsInfo == 2) {
         ...
         if (SymbolPair[i] != "") {
            ...
            //---Background1
            if ((i < NumberPairsTrade / 2) && (MathMod(NumberPairsTrade, 2) == 0))
               if (ObjectFind("BackgroundLine1" + IntegerToString(i)) == -1) ChartBackground("BackgroundLine1" + IntegerToString(i), ColorOfLine1, EMPTY_VALUE, TRUE, 245, 22 + (i * 14 * 2), PositionSpread - 245 + 55, 16);
            if ((i <= NumberPairsTrade / 2) && (MathMod(NumberPairsTrade, 2) == 1))
               if (ObjectFind("BackgroundLine1" + IntegerToString(i)) == -1) ChartBackground("BackgroundLine1" + IntegerToString(i), ColorOfLine1, EMPTY_VALUE, TRUE, 245, 22 + (i * 14 * 2), PositionSpread - 245 + 55, 16);
            //---Background2
            if (i < NumberPairsTrade / 2)
               if (ObjectFind("BackgroundLine2" + IntegerToString(i)) == -1) ChartBackground("BackgroundLine2" + IntegerToString(i), ColorOfLine2, EMPTY_VALUE, TRUE, 245, 36 + (i * 14 * 2), PositionSpread - 245 + 55, 16);
         }
      }
   }
   //---------------------------------------------------------------------
   ...
   Comment("=================================" +
      ...
      "\n=================================" +
      ShowInfoOfPairs +
      "\n  Orders: " + DoubleToStr(TotalOrders, 0) + " (" + DoubleToStr(MaxTotalOrders, 0) + ")" + " | Lots: " + DoubleToStr(TotalLots, 2) + " | PnL: " + DoubleToStr(TotalProfitLoss, 2) + " (" + DoubleToStr(ShowMinProfit, 2) + ")" +
      "\n=================================" +
      "\n  Total HIstory Results" +
      "\n  Pips: " + DoubleToStr(HistoryTotalPips, 2) + " | Orders: " + DoubleToStr(HistoryTotalTrades, 0) + " | PnL: " + DoubleToStr(HistoryTotalProfitLoss, 2) +
      "\n=================================" +
      "\n" +
      "\n  Balance: " + DoubleToStr(AccountBalance(), 2) +
      "\n  Equity: " + DoubleToStr(AccountEquity(), 2) +
      "\n  Floating: " + DoubleToStr((AccountEquity() - AccountBalance()), 2) +
      "\n---------------------------------" +
      "\n  Equity Min: " + DoubleToStr(EquityMin, 2) +
      "\n  Equity Max: " + DoubleToStr(EquityMax, 2) +
      "\n  Floating Min: " + DoubleToStr(FloatingMin, 2) +
      "\n  Floating Max: " + DoubleToStr(FloatingMax, 2));
   //---------------------------------------------------------------------
}
//====================================================================================================================================================//
//End of code
...
void RemoveAllOrders() {
   int TicketOrderSelect, TicketOrderDelete, TicketOrderClose;
