...
extern double Stoploss = 49.9;
extern double TrailStart = 11.1;
extern double TrailStop = 11.1;

extern double LotExponent = 1.3;
extern int slip = 3.0;
extern double Lots = 0.01;
extern int lotdecimal = 2;
extern double TakeProfit = 99.9;
extern double PipStep = 49.9;
extern int MagicNumber = 5758;
extern string CommentMagic = "Bismillahirrohmanirrohim";
extern double CloseAllEquityUnder = 9900.0;
...
int start() {
   ...
   if(AccountEquity() < CloseAllEquityUnder) {
      CloseThisSymbolAll();
      CloseThisSymbolAll();
      CloseThisSymbolAll();
      CloseThisSymbolAll();
      CloseThisSymbolAll();
      Print("Closed All due to LowEquity");
      ExpertRemove();
   }
   ...
   if (TradeNow) {
      ...
      if (ShortTrade) {
         ...
         RefreshRates();
         ticket = OpenPendingOrder(1, iLots, Bid, slip, Ask, 0, 0, CommentMagic, MagicNumber, 0, HotPink);
         if (ticket < 0) {
         ...
         if (LongTrade) {
            NumOfTrades = total;
            iLots = NormalizeDouble(Lots * MathPow(LotExponent, NumOfTrades), lotdecimal);
            ticket = OpenPendingOrder(0, iLots, Ask, slip, Bid, 0, 0, CommentMagic, MagicNumber, 0, Lime);
            if (ticket < 0) {
               Print("Error: ", GetLastError());
               return (0);
            }
      ...
      if (!ShortTrade && !LongTrade) {
         ...
         if (PrevCl > CurrCl) {
            if (iRSI(NULL, PERIOD_H4, 14, PRICE_CLOSE, 1) > 10.0) {
               ticket = OpenPendingOrder(1, iLots, SellLimit, slip, SellLimit, 0, 0, CommentMagic, MagicNumber, 0, HotPink);
               if (ticket < 0) {
                  Print("Error: ", GetLastError());
                  return (0);
               }
            ...
            if (iRSI(NULL, PERIOD_H4, 14, PRICE_CLOSE, 1) < 90.0) {
               ticket = OpenPendingOrder(0, iLots, BuyLimit, slip, BuyLimit, 0, 0, CommentMagic, MagicNumber, 0, Lime);
               if (ticket < 0) {
                  Print("Error: ", GetLastError());
                  return (0);
               }
