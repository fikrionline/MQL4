...
input int TimeFrameInMinutes = 120;
input color UpCandleColor = clrBlue;
...
int OnInit() {
   ...
   if (Period() != PERIOD_H1) {
      Comment("This indicator works in H1 Time Frame!");
      //return (INIT_FAILED);
   }
   if (MathMod(TimeFrameInMinutes, Period()) != 0) {
      Comment("Valid Time Frames are " + ValidTimeFrames);
      return (INIT_FAILED);
   }
...
void DrawClosedCandle(int i, int g) {
   ...
   string boxName = "obj" + IntegerToString(obj_counter);
   obj_counter++;
   ...
   string highStickName = "obj" + IntegerToString(obj_counter);
   obj_counter++;
   string lowStickName = "obj" + IntegerToString(obj_counter);
   obj_counter++;
