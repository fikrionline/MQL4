...
void Draw_H1_Lines() {
   ...
   int PipBar, pip, idx;
   datetime PipTime;
   double pips, pip_price, Line_price, thepoint, pip_dir, H, L;
   ...
   for (pip = 1; pip <= NumPipLines; pip++) {
      pips = (PipIncrement * pip) * thepoint;
      Line_price = pip_price + (pips * pip_dir);
      objectName = sObj + "piplines" + IntegerToString(pip, 0);
...
void Show_TimeInfos() {
   ...
   if (Period() <= PERIOD_D1) {
      sTimeLeft = IntegerToString(h, 0) + "h " + IntegerToString(m, 0) + "m " + IntegerToString(s, 0) + "s";
   } else {
      sTimeLeft = IntegerToString(d, 0) + "d " + IntegerToString(h, 0) + "h " + IntegerToString(m, 0) + "m ";
   }
   ...
   Comment(
      "$$$TooSlow Version$$$",
      "\n" + sIndiName,
      "\nTime Local = ", TimeToString(timeLocal, TIME_DATE | TIME_MINUTES),
      "\nTime Server = ", TimeToString(timeServer, TIME_DATE | TIME_MINUTES),
...
void Draw_TrendLine() {
   ...
   if (Trend_Bars > 0) {
      ...
      BarsInARow = TBars;
      if (TDirection) {
         InARow = IntegerToString(BarsInARow, 0) + " Higher opens in a row";
      } else {
         InARow =  IntegerToString(BarsInARow, 0) + " Lower opens in a row";
      }
   }
