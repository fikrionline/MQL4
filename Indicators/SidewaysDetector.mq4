...
// Color of the zone
extern color Rectangle_Color = clrGray;
// Number of last zones to show. 0 = Show all zones
extern int Show_N_Last_Zones = 99;
// Number of bars to process. 0 = All
extern int Num_Bars = 999;

double BufferName[], BufferZoneHighest[], BufferZoneLowest[], BufferZoneOCtotal[], BufferZoneHLtotal[], BufferZoneBarStart[], BufferZoneBarEnd[];

int iBarShiftStart, iBarShiftEnd, OCHLcounter;
double OCbullish, OCbearish, OCtotal, HLbullish, HLbearish, HLtotal;
color Rectangle_Color_Zone;
...
int OnInit() {
   ...
   IndicatorShortName("SidewayDetector(" + GetTimeFrame(Period()) + ")");
   
   IndicatorBuffers(7);
   SetIndexBuffer(0, BufferName);
   SetIndexBuffer(1, BufferZoneHighest);
   SetIndexLabel(1, "ZoneHighest");
   SetIndexBuffer(2, BufferZoneLowest);
   SetIndexLabel(2, "ZoneLowest");
   SetIndexBuffer(3, BufferZoneOCtotal);
   SetIndexLabel(3, "OCTotal");
   SetIndexBuffer(4, BufferZoneHLtotal);
   SetIndexLabel(4, "HLTotal");
   SetIndexBuffer(5, BufferZoneBarStart);
   SetIndexLabel(5, "BarStart");
   SetIndexBuffer(6, BufferZoneBarEnd);
   SetIndexLabel(6, "BarEnd");
...
void DrawZones(Zone & zones[]) {
   ...
   for (int i = 0; i < zonesCounter; i++) {
      ...
      iBarShiftStart = iBarShift(Symbol(), PERIOD_CURRENT, Time[zone.Start]);
      iBarShiftEnd = iBarShift(Symbol(), PERIOD_CURRENT, Time[zone.End]);
      ...
      for(OCHLcounter = iBarShiftEnd; OCHLcounter <= iBarShiftStart; OCHLcounter++) {
      ...
      if(OCtotal > 0 && HLtotal > 0) {
         Rectangle_Color_Zone = clrNavy;
      } else if(OCtotal < 0 && HLtotal < 0) {
         Rectangle_Color_Zone = clrMaroon;
      } else {
         Rectangle_Color_Zone = Rectangle_Color;
      }
      
      ObjectCreate(objName, OBJ_RECTANGLE, 0, Time[zone.Start], zone.High, Time[zone.End], zone.Low);
      ObjectSetInteger(0, objName, OBJPROP_COLOR, Rectangle_Color_Zone);
      ObjectSetInteger(0, objName, OBJPROP_BACK, 1);
      
      ObjectCreate(objName + "_text", OBJ_TEXT, 0, Time[zone.Start], zone.Low, Time[zone.End], zone.High);
      ObjectSetText(objName + "_text", DoubleToString(OCtotal, 0) + "/" + DoubleToString(HLtotal, 0), 8, "Arial", clrYellow);
      
      BufferZoneHighest[0] = NormalizeDouble(zone.High, Digits());
      BufferZoneLowest[0] = NormalizeDouble(zone.Low, Digits());
      BufferZoneOCtotal[0] = NormalizeDouble(OCtotal, 0);
      BufferZoneHLtotal[0] = NormalizeDouble(HLtotal, 0);
      BufferZoneBarStart[0] = iBarShiftStart;
      BufferZoneBarEnd[0] = iBarShiftEnd;
