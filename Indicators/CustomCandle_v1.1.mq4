...
//+------------------------------------------------------------------------------------------------+
int init() {
   IndicatorShortName("M" + SetTF + " íà M" + Period());
   Nbar = SetTF / Period();
   timer = Period() * 60;
   timerTF = SetTF * 60;
   name = 0;
...
int start() {
   ...
   if (SetTF <= Period() || MathMod(SetTF, Period()) != 0) {
      Comment("\n", " TimeFrame should be more or divisible by M", Period());
      return (0);
   }
   ...
   while (i > 0) {
      i--;
      while (i >= 0)
         if (Time[i] == TimeOpenNewBar || BarNew(i, SetTF) == false) i--;
         else {
            NewBar = true;
            name++;
            break;
