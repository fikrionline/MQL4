...
int start() {
   ...
   //---- 
   if ((AllBars == 0) || (Bars < AllBars)) AB = Bars;
   ...
   if ((B1 == -1) || (B2 == -1)) {
      DelObj();
      return (-1);
   } // Çíà÷èò íå íàøëè ôðàêòàëîâ ñðåäè 300 áàðîâ 8-)
   ...
   ObjectSet("TL1", OBJPROP_COLOR, clrYellow);
   ObjectSet("TL1", OBJPROP_WIDTH, 1);
   ObjectSet("TL1", OBJPROP_STYLE, STYLE_DOT);
   ObjectCreate("TL2", OBJ_TREND, 0, T2, P2, T1, P1);
   ObjectSet("TL2", OBJPROP_COLOR, clrYellow);
   ObjectSet("TL2", OBJPROP_WIDTH, 1);
   ObjectSet("TL2", OBJPROP_STYLE, STYLE_DOT);
   ObjectCreate("MIDL", OBJ_TREND, 0, T2, (P2 + PP + Step * AB) / 2, T1, (P1 + PP) / 2);
   ObjectSet("MIDL", OBJPROP_COLOR, clrYellow);
