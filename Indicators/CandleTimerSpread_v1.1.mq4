...
void ShowClock() {
   ...
   ObjectSetString(0, clockName, OBJPROP_TEXT, time + " / " + DoubleToString(MarketInfo(NULL, MODE_SPREAD), 0));
   ...
   if (ChartGetInteger(0, CHART_SHIFT, 0) == 0 && (shift >= 0)) {
      ...
      ObjectSetInteger(0, clockName, OBJPROP_TIME, barTime + shift + CandleShift * _Period * 60);
   }
   ...
   if (visible && price >= ChartGetDouble(0, CHART_PRICE_MAX, 0)) {
      ...
      ObjectSetDouble(0, clockName, OBJPROP_PRICE, price);
   }
}
...
string getTime(int times, color & theColor) {
   ...
   if (times < 0) {
      theColor = NegativeColor;
      times = (int) fabs(times);
   } else {
      theColor = PositiveColor;
   }
   seconds = (times % 60);
