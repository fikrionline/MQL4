...
extern enMaTypes AverageType         = ma_sma;
extern int       SF                  = 5;
extern int       RSIPeriod           = 14;
extern enPrices  RSIPrice            = pr_close;
extern int       RSIPriceFilter      = 10;      
extern enMaTypes RSIPriceFilterType  = ma_sma;      
...
int init() {
   ...
   SetIndexBuffer(1, HistoM);
   SetIndexStyle(1, DRAW_HISTOGRAM);
   SetIndexLabel(1, NULL);
   SetIndexBuffer(2, HistoD);
   SetIndexStyle(2, DRAW_HISTOGRAM);
   SetIndexLabel(2, NULL);
   SetIndexBuffer(3, RsiMa);
   SetIndexLabel(3, NULL);
   SetIndexBuffer(4, Trend);
   SetIndexLabel(4, "QQEModTrend");
...
double iSma(double price, int period, int r, int instanceNo = 0) {
   ...
   if (r >= period)
      ...
      for (int k = 0; k < period && (r - k) >= 0; k++) workSma[r][instanceNo + 1] += workSma[r - k][instanceNo];
...
double iDsema(double price, double period, int r, int instanceNo = 0) {
   if (ArrayRange(workDsema, 0) != Bars) ArrayResize(workDsema, Bars);
   instanceNo *= 2;
...
double iTma(double price, double period, int r, int instanceNo = 0) {
   ...
   for (int k = 1; k < period && (r - k) >= 0; k++) {
      double weight = k + 1;
      if (weight > half) weight = period - k;
      sumw += weight;
...
double iLeader(double price, double period, int r, int instanceNo = 0) {
   if (ArrayRange(workLeader, 0) != Bars) ArrayResize(workLeader, Bars);
   instanceNo *= 2;
...
double iSmooth(double price, int length, int r, int instanceNo = 0) {
   if (ArrayRange(workSmooth, 0) != Bars) ArrayResize(workSmooth, Bars);
   instanceNo *= 5;
   if (r <= 2) {
