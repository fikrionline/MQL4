...
extern int                UpArrowSize     = 1;                // Up Arrow size
extern int                DnArrowSize     = 1;                // Down Arrow size
extern int                UpArrowCode     = 225;              // Up Arrow code
extern int                DnArrowCode     = 226;              // Down arrow code
extern double             UpArrowGap      = 0.5;              // Up Arrow gap        
extern double             DnArrowGap      = 0.5;              // Dn Arrow gap
extern color              UpArrowColor    = clrBlue;     // Up Arrow Color
...
int init() {
   ...
   if (DisplayType == en_dot) lstyle = DRAW_NONE;
   int astyle = DRAW_ARROW;
   if (DisplayType < en_lid) astyle = DRAW_NONE;
...
int start() {
   ...
   if (TimeFrame != _Period) {
      ...
      for (i = limit; i >= 0 && !_StopFlag; i--) {
         ...
         if (ArrowOnFirst) {
            ...
            if (i > 0) x = iBarShift(NULL, TimeFrame, Time[i - 1]);
            else x = -1;
         }
         jur[i] = _mtfCall(0, y);
         jurDa[i] = EMPTY_VALUE;
   ...
   if (FilterType == flt_val) pfilter = 0;
   double vfilter = Filter;
   if (FilterType == flt_prc) vfilter = 0;
   for (i = limit; i >= 0; i--) {
...
double iSmooth(double price, double length, double phase, int i, int s = 0) {
   ...
   if (del1 > 0) wrk[r][bsmax + s] = price;
   else wrk[r][bsmax + s] = price - Kv * del1;
   if (del2 < 0) wrk[r][bsmin + s] = price;
   else wrk[r][bsmin + s] = price - Kv * del2;
