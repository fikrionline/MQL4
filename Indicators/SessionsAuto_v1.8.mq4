...
// timginter @ ForexFactory --> https://www.forexfactory.com/thread/480761-automatic-session-boxes-indicator
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
void init() {
   ...
   TO = ConvertTime(3 + DSTOffset + brokerDiff) * 3600;
   ...
   for (int dayNr = days; dayNr >= daysLimit; dayNr--) {
      ...
      if (TimeDayOfWeek(dayTime - brokerDiff * 3600) <= 5) {
         ...
         if (London != CLR_NONE && London != 2147483647) {
            Session("sa_LO_" + dayNr, LO + dayTime, StrToInteger(sLOMod[0]), StrToInteger(sLOMod[1]));
         }
         if (NewYork != CLR_NONE && NewYork != 2147483647) {
            Session("sa_NY_" + dayNr, NY + dayTime, StrToInteger(sNYMod[0]), StrToInteger(sNYMod[1]));
         }
         if (Sydney != CLR_NONE && Sydney != 2147483647) {
            Session("sa_SY_" + dayNr, SY + dayTime, StrToInteger(sSYMod[0]), StrToInteger(sSYMod[1]));
         }
         if (Tokyo != CLR_NONE && Tokyo != 2147483647) {
            Session("sa_TO_" + dayNr, TO + dayTime, StrToInteger(sTOMod[0]), StrToInteger(sTOMod[1]));
         }
      }
   }
}
...
void start() {
   ...
   if (London != CLR_NONE && London != 2147483647) {
      Session("sa_LO_" + days, LO + dayTime, StrToInteger(sLOMod[0]), StrToInteger(sLOMod[1]));
   }
   if (NewYork != CLR_NONE && NewYork != 2147483647) {
      Session("sa_NY_" + days, NY + dayTime, StrToInteger(sNYMod[0]), StrToInteger(sNYMod[1]));
   }
   if (Sydney != CLR_NONE && Sydney != 2147483647) {
      Session("sa_SY_" + days, SY + dayTime, StrToInteger(sSYMod[0]), StrToInteger(sSYMod[1]));
   }
   if (Tokyo != CLR_NONE && Tokyo != 2147483647) {
      Session("sa_TO_" + days, TO + dayTime, StrToInteger(sTOMod[0]), StrToInteger(sTOMod[1]));
   }
}
...
void Session(string id, datetime begin, datetime bMOD, datetime eMOD) {
   ...
   // get session colour
   if (StringFind(id, "sa_LO_") >= 0) {
      colour = London;
   } else if (StringFind(id, "sa_NY_") >= 0) {
      colour = NewYork;
   } else if (StringFind(id, "sa_SY_") >= 0) {
      colour = Sydney;
   } else if (StringFind(id, "sa_TO_") >= 0) {
      colour = Tokyo;
   }
