...
//---- input parameters
extern double TakeProfitPips = 50;
extern double StopLossPips = 50;
extern int LayerOrder = 4;
extern double LayerPips = 53;
    ...
    if (OrdersTotal() != 0)
    {
        ...
        for (cnt = 0; cnt < OrdersTotal(); cnt++)
        {
                ...
                && StopLossPips != 0)
            {
                ...
                {
                    SelectOrder = OrderModify(OrderTicket(), OrderOpenPrice(), SLp, TPp, 0);
                }
                //--------------------------------------------------------
                ...
                if (OrderType() == OP_BUY)
                {
                    for (lyr = 1; lyr <= LayerOrder; lyr++)
                    {
                        ...
                        NewOrder = OrderSend(Symbol(), OP_BUYLIMIT, OrderLots() * multiplier,
                            OrderOpenPrice() - ((LayerPips * lyr) / Faktor), 0, SLp, TPp, NULL,
