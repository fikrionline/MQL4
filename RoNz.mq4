...
input int TakeProfit = 407; //Take Profit
input int StopLoss = 143; //Stop Loss
...
void OrderTest() {
   ...
   int CheckLastClosedOrder = LastClosedOrder(MagicNumber);

   if (GetSignal() == 1) {
      if(CheckLastClosedOrder == -1 || CheckLastClosedOrder == 0) {
         int ticket = OrderSend(NULL, OP_BUY, LotSize, Ask, SlipPage, 0, 0, IntegerToString(MagicNumber), MagicNumber, 0, clrNONE);
      }
   }
   
   if (GetSignal() == -1) {
      if(CheckLastClosedOrder == 1 || CheckLastClosedOrder == 0) {
         int ticket = OrderSend(NULL, OP_SELL, LotSize, Bid, SlipPage, 0, 0, IntegerToString(MagicNumber), MagicNumber, 0, clrNONE);
      }
   }
...
int GetSignal() {
   ...
   double iRSI_value;
   int iRSI_signal;
   iRSI_value = iRSI(NULL, PERIOD_CURRENT, 3, PRICE_OPEN, 0);
   if(iRSI_value <= 5) {
      iRSI_signal = 1;
   } else if(iRSI_value >= 95) {
      iRSI_signal = -1;
   } else {
      iRSI_signal = 0;
   }
