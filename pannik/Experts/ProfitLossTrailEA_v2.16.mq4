...
//Set background
   ...
   if(ObjectFind(BackgroundName)==-1) ChartBackground(BackgroundName,(color)ChartColor,0,15,135,179);
//------------------------------------------------------
...
//------------------------------------------------------
//External comment
   if(PutTakeProfit==true) TP=DoubleToStr(TakeProfitPips,2); else TP="FALSE";
   if(PutStopLoss==true) SL=DoubleToStr(StopLossPips,2); else SL="FALSE";
   if(UseTrailingStop==true) TSL=DoubleToStr(TrailingStop,2)+"  ("+DoubleToStr(PutStopLossAfter,2)+")"; else TSL="FALSE";
   if(UseBreakEven==true) BE=DoubleToStr(BreakEvenPips,2)+"  ("+DoubleToStr(BreakEvenAfter,2)+")"; else BE="FALSE";
   ...
   if(SoundAlert==true) SA="TRUE"; else SA="FALSE";
//----------------------------------
//Set manage mode
               ...
               if(ManageOrders==0)
                 {
                  ...
                  if((PutTakeProfit==true) && (OrderTakeProfit()==0))
                     LocalTakeProfit=NormalizeDouble(PriceBuyAsk+TakeProfitPips*(MarketInfo(OrderSymbol(),MODE_POINT)*MultiplierPoint),DigitsPrices); else LocalTakeProfit=OrderTakeProfit();
                 }
               //---
               if(ManageOrders==1)
                 {
               ...
               if(ManageOrders==0)
                 {
                  ...
                  if((PutTakeProfit==true) && (OrderTakeProfit()==0))
                     LocalTakeProfit=NormalizeDouble(PriceSellBid-TakeProfitPips*(MarketInfo(OrderSymbol(),MODE_POINT)*MultiplierPoint),DigitsPrices); else LocalTakeProfit=OrderTakeProfit();
                 }
               //---
               if(ManageOrders==1)
                 {
