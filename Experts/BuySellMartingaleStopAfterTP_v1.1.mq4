...
extern BuySellChoose BuyOrSell = BuyOnly;
...
extern double PipStepDevideADR = 18;
extern double PipStepMultiplier = 1;
extern double TakeProfitDevideADR = 18;
...
int TicketOrderSend, TicketOrderSelect, TicketOrderModify, TicketOrderClose, TicketOrderDelete, TotalOrderBuy, TotalOrderSell, LastTicket, LastTicketTemp, NumOfTradesSell, NumOfTradesBuy, MagicNumberBuy, MagicNumberSell, cnt, TheHistoryTotal;
...
bool NewOrdersPlacedBuy = FALSE, NewOrdersPlacedSell = FALSE, FirstOrderBuy = FALSE, FirstOrderSell = FALSE, StopOrderBuy = FALSE, StopOrderSell = FALSE;
...
int start() {
   ...
   if (LastTradeBarTime == Time[0]) {
      ...
      if(BuyOrSell == BuyOnly || BuyOrSell == BuyAndSell) {
         ...
         if (TotalOrderBuy < 1 && StopOrderBuy == FALSE) {
            NumOfTradesBuy = 0;
            ...
            if ((Hour() >= StartHour && Hour() < EndHour)) {
               ...
               RefreshRates();
               TicketOrderSend = OrderSend(Symbol(), OP_BUY, iLotsBuy, Ask, SlipPage, 0, FirstTPOrderBuy, Symbol() + "-" + NumOfTradesBuy, MagicNumberBuy, 0, Lime);
               Print(Symbol() + "-" + NumOfTradesBuy + "_MN-" + MagicNumberBuy + "_FirstTP");
               //Sleep(1111);
         ...
         if (TotalOrderBuy > 0 && TotalOrderBuy < MaxTrades) {
            ...
            if ((LastBuyPrice - Ask) >= (LastPipStepMultiplierBuy * Point)) {
               ...
               iLotsBuy = NormalizeDouble(StartingLots * MathPow(LotsMultiplier, NumOfTradesBuy), 2);
               RefreshRates();
               TicketOrderSend = OrderSend(Symbol(), OP_BUY, iLotsBuy, Ask, SlipPage, 0, 0, Symbol() + "-" + NumOfTradesBuy, MagicNumberBuy, 0, Lime);
               Print(Symbol() + "-" + NumOfTradesBuy + "_MN-" + MagicNumberBuy);
               //Sleep(1111);
               if (TicketOrderSend < 0) {
                  Print("Error: ", GetLastError());
               }
      ...
      if(BuyOrSell == SellOnly || BuyOrSell == BuyAndSell) {
         ...
         if (TotalOrderSell < 1  && StopOrderSell == FALSE) {
            NumOfTradesSell = 0;
            ...
            if ((Hour() >= StartHour && Hour() <= EndHour)) {
               ...
               RefreshRates();
               TicketOrderSend = OrderSend(Symbol(), OP_SELL, iLotsSell, Bid, SlipPage, 0, FirstTPOrderSell, Symbol() + "-" + NumOfTradesSell, MagicNumberSell, 0, Lime);
               Print(Symbol() + "-" + NumOfTradesSell + "_MN-" + MagicNumberSell + "_FirstTP");
               //Sleep(1111);
               if (TicketOrderSend < 0) {
                  Print("Error: ", GetLastError());
               }
         ...
         if (TotalOrderSell > 0 && TotalOrderSell < MaxTrades) {
            ...
            if ((Bid - LastSellPrice) >= (LastPipStepMultiplierSell * Point)) {
               ...
               iLotsSell = NormalizeDouble(StartingLots * MathPow(LotsMultiplier, NumOfTradesSell), 2);
               RefreshRates();
               TicketOrderSend = OrderSend(Symbol(), OP_SELL, iLotsSell, Bid, SlipPage, 0, 0, Symbol() + "-" + NumOfTradesSell, MagicNumberSell, 0, Lime);
               Print(Symbol() + "-" + NumOfTradesSell + "_MN-" + MagicNumberSell);
               //Sleep(1111);
               if (TicketOrderSend < 0) {
                  Print("Error: ", GetLastError());
               }
