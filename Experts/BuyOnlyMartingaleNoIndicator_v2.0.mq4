...
extern PipStepDevideADR PipStepDevide = pb;
...
int TicketOrderSend, TicketOrderSelect, TicketOrderModify, TicketOrderClose, TicketOrderDelete, TotalOrderBuy, LastTicket, LastTicketTemp, NumOfTradesBuy, MagicNumberBuy, cnt;
double PriceTargetBuy, AveragePriceBuy, LastBuyPrice, iLotsBuy, Multiplier, ADRs, PipStep, PipStepDevideResult, TakeProfit, TakeProfitDevideResult, StartEquityBuy, Count, LastPipStepMultiplierBuy;
bool NewOrdersPlacedBuy = FALSE, FirstOrderBuy = FALSE;
...
void OnTick() {
   ...
   if(TotalOrderBuy >= BEPHunterOnLayer && AccountEquity() > (StartEquityBuy + 1)) {
      RemoveAllOrders();
      TotalOrderBuy = 0;
   }
   
   if(TotalOrderBuy < 1) {
      StartEquityBuy = DoubleToString(AccountEquity(), 2);
      NumOfTradesBuy = 0;
