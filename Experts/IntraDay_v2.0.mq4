...
int GetSignal() {
   ...
   if(iCustom(Symbol(), PERIOD_CURRENT, "FollowLine", 5, 1) != EMPTY_VALUE) {
      SignalFollowLineStart = 1;
   }
   if(iCustom(Symbol(), PERIOD_CURRENT, "FollowLine", 6, 1) != EMPTY_VALUE) {
      SignalFollowLineStart = -1;
   }
   
   if(SignalFollowLineStart == 1) {
      SignalResult = 1;
   }
