...
// ------------------------------------------------------------------------------------------------
extern int user_slippage = 5;
//extern int grid_size = 20; // Grid_Size
extern double TheGridSize = 5;
...
double min_equity = 0; // minimum of equity ever reached, saved in global vars
...
void OnTick() {
   ...
   //grid_size = NormalizeDouble((GetADRs(PERIOD_D1, 20, 1) / TheGridSize) / 10, 2);
...
void ShowData() {
   ...
   txt +=
      ...
      "\nMin. Equity: " + DoubleToString(min_equity, 2) + " " + ter_currencySymbol +
...
void RemoveAllOrders() {
   int FunctionTicketOrderSelect, FunctionTicketOrderClose, FunctionTicketOrderDelete;
   for(int i = OrdersTotal() - 1; i >= 0 ; i--)    {
      FunctionTicketOrderSelect = OrderSelect(i, SELECT_BY_POS);
      if(OrderType() == OP_BUY) {
         FunctionTicketOrderClose = OrderClose(OrderTicket(), OrderLots(), MarketInfo(OrderSymbol(), MODE_BID), 5, clrNONE);
      } else if(OrderType() == OP_SELL) {
         FunctionTicketOrderClose = OrderClose(OrderTicket(), OrderLots(), MarketInfo(OrderSymbol(), MODE_ASK), 5, clrNONE);
      } else {
         FunctionTicketOrderDelete = OrderDelete(OrderTicket());
      }
