...
extern int TheMinZoneBar = 5;
extern int TheMaxZoneBar = 100;
extern double MinBoxSize = 50;
...
void OnTick() {
   ...
   if (NextCandle <= Time[0]) {
      ...
      if(Hour() >= StartHour && Hour() < EndHour) {
         ...
         ZoneBarEnd = iCustom(Symbol(), PERIOD_CURRENT, "SidewaysDetector", 6, 0); //Print("Start_" + DoubleToString(ZoneBarStart, 0) + "_End_" + DoubleToString(ZoneBarEnd, 0) + "_" + DoubleToString((ZoneHighest - ZoneLowest) / Point(), 0));
         
         if((ZoneHighest - ZoneLowest) / Point() > MinBoxSize && (ZoneHighest - ZoneLowest) / Point() < MaxBoxSize) {
         
            NewSignal = 0;
            
            if(InverseOrder == false) {
               if(ZoneOCTotal > 0 && ZoneHLTotal > 0) {
                  NewSignal = 1;
               } else if(ZoneOCTotal < 0 && ZoneHLTotal < 0) {
                  NewSignal = -1;
               }
            }
         ...
         if (ZoneBarEnd == TheZoneBarStart && ZoneBarStart >= TheMinZoneBar && ZoneBarStart <= TheMaxZoneBar) {
            ...
            if(NewSignal == 1 && PosSelect(MagicNumber) == 0) {
               ...
               if((Ask - StopLoss) / Point < MinSL) {
                  StopLoss = Ask - (MinSL * Point); //Print("SL --> " + DoubleToString((Ask - ZoneLowest) / Point, 0) + " --> " + DoubleToString((Ask - StopLoss) / Point, 0));
               }
            ...
            if(NewSignal == -1 && PosSelect(MagicNumber) == 0) {
               ...
               if((StopLoss - Bid) / Point < MinSL) {
                  StopLoss = Bid + (MinSL * Point); //Print("SL --> " + DoubleToString((ZoneHighest - Bid) / Point, 0) + " --> " + DoubleToString((StopLoss - Bid) / Point, 0));
               }
