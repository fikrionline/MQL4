...
void OnTick() {
   ...
   if(CanTradeNow(MagicNumber)) {
      ...
      if(Hour() == OpenHour && Minute() == OpenMinute) {
         ...
         double PeriodHighest, PeriodLowest, PriceBuyLimit, PriceSellLimit, SLBuy, SLSell, TPBuy, TPSell, OCbullish = 0, OCbearish = 0, OCtotal = 0, HLbullish = 0, HLbearish = 0, HLtotal = 0;
         ...
         if(((PeriodHighest - PeriodLowest) / Point) > MinBoxSize && ((PeriodHighest - PeriodLowest) / Point) < MaxBoxSize) {
            ...
            PriceBuyLimit = PeriodLowest;
            PriceSellLimit = PeriodHighest;
            
            SLBuy = PeriodLowest - (PeriodHighest - PeriodLowest) + (PlusMinusTPSL * Point);
            SLSell = PeriodHighest + (PeriodHighest - PeriodLowest) - (PlusMinusTPSL * Point);
            
            if(RiskReward == RRZeroPointFive) {
               TPBuy = PeriodLowest + ((PeriodHighest - PeriodLowest) / 2) + (PlusMinusTPSL * Point);
               TPSell = PeriodHighest - ((PeriodHighest - PeriodLowest) / 2) - (PlusMinusTPSL * Point);
            } else if(RiskReward == RROne) {
               TPBuy = PeriodLowest + (PeriodHighest - PeriodLowest) + (PlusMinusTPSL * Point);
               TPSell = PeriodHighest - (PeriodHighest - PeriodLowest) - (PlusMinusTPSL * Point);
            } else if(RiskReward == RRTwo) {
               TPBuy = PeriodLowest + ((PeriodHighest - PeriodLowest) * 2) + (PlusMinusTPSL * Point);
               TPSell = PeriodHighest - ((PeriodHighest - PeriodLowest) * 2) - (PlusMinusTPSL * Point);
            } else if(RiskReward == RRThree) {
               TPBuy = PeriodLowest + ((PeriodHighest - PeriodLowest) * 3) + (PlusMinusTPSL * Point);
               TPSell = PeriodHighest - ((PeriodHighest - PeriodLowest) * 3) - (PlusMinusTPSL * Point);
            } else { //Other is 1:1
               TPBuy = PeriodLowest + (PeriodHighest - PeriodLowest) + (PlusMinusTPSL * Point);
               TPSell = PeriodHighest - (PeriodHighest - PeriodLowest) - (PlusMinusTPSL * Point);
            }
            ...
            if(PosSelectBuyLimit(MagicNumber) == 0) {
               TicketBuy = OrderSend(Symbol(), OP_BUYLIMIT, Lots, PriceBuyLimit, SlipPage, SLBuy, TPBuy, "Bismillahirrohmanirrohim_BUY", MagicNumber, 0, clrBlue);
            }
            
            if(PosSelectSellLimit(MagicNumber) == 0) {
               TicketSell = OrderSend(Symbol(), OP_SELLLIMIT, Lots, PriceSellLimit, SlipPage, SLSell, TPSell, "Bismillahirrohmanirrohim_SELL", MagicNumber, 0, clrRed);
            }
