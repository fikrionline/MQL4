      ...
      NewSignal = GetSignal();
         ...
         if (PosSelect(MagicNumber) == 0) {
            ...
            if(NewSignal == 1 && iOpen(Symbol(), PERIOD_CURRENT, 1) < iClose(Symbol(), PERIOD_CURRENT, 1)) {
               ...
               StopLoss = NormalizeDouble(iCustom(Symbol(), PERIOD_CURRENT, "FollowLineIndicator", 3, 1), Digits());
               
               if(RiskReward == RRZeroPointFive) {
                  TakeProfit = Ask + ((Ask - StopLoss) / 2);
               } else if(RiskReward == RROne) {
                  TakeProfit = Ask + (Ask - StopLoss);
               } else if(RiskReward == RRTwo) {
                  TakeProfit = Ask + ((Ask - StopLoss) * 2);
               } else if(RiskReward == RRThree) {
                  TakeProfit = Ask + ((Ask - StopLoss) * 3);
               } else { //Other is 1:1
                  TakeProfit = Ask + (Ask - StopLoss);
               }
               ...
               StopLoss = NormalizeDouble(iCustom(Symbol(), PERIOD_CURRENT, "FollowLineIndicator", 4, 1), Digits());
               
               if(RiskReward == RRZeroPointFive) {
                  TakeProfit = Bid - ((StopLoss - Bid) / 2);
               } else if(RiskReward == RROne) {
                  TakeProfit = Bid - (StopLoss - Bid);
               } else if(RiskReward == RRTwo) {
                  TakeProfit = Bid - ((StopLoss - Bid) * 2);
               } else if(RiskReward == RRThree) {
                  TakeProfit = Bid - ((StopLoss - Bid) * 3);
               } else { //Other is 1:1
                  TakeProfit = Bid - (StopLoss - Bid);
               }
...
int GetSignal() {
   ...
   int SignalResult = 0, SignalFollowLineIndicator = 0, SignalHullSuite = 0, SignalQQEMod = 0;
   
   if(iCustom(Symbol(), PERIOD_CURRENT, "FollowLineIndicator", 3, 1) != EMPTY_VALUE) {
      SignalFollowLineIndicator = 1;
   }
   if(iCustom(Symbol(), PERIOD_CURRENT, "FollowLineIndicator", 4, 1) != EMPTY_VALUE) {
      SignalFollowLineIndicator = -1;
   }
   ...
   if(iCustom(Symbol(), PERIOD_CURRENT, "QQEMod", 4, 1) >= 0) {
      SignalQQEMod = 1;
   }
   if(iCustom(Symbol(), PERIOD_CURRENT, "QQEMod", 4, 1) <= 0) {
      SignalQQEMod = -1;
   ...
   if(SignalFollowLineIndicator == 1 && SignalHullSuite == 1 && SignalQQEMod == 1) {
      SignalResult = 1;
   }
