...
int init() {
   ...
   if (DigitsEuro <= 4) {
      OneOverPoint = OneOverPoint / 10;
      point = Point / 10.0;
   } else {
      OneOverPoint = OneOverPoint;
      point = Point;
   }
...
int start() {
   ...
   for (int l_pos_16 = 0; l_pos_16 <= OrdersTotal() - 1; l_pos_16++) {
      SelectOrder = OrderSelect(l_pos_16, SELECT_BY_POS, MODE_TRADES);
   ...
   if (TrailSL == true) {
      if (CountTradesB() == 1) {
         for (int l_pos_20 = 0; l_pos_20 <= OrdersTotal() - 1; l_pos_20++) {
            SelectOrder = OrderSelect(l_pos_20, SELECT_BY_POS, MODE_TRADES);
                  ...
                  if (Bid - OrderOpenPrice() >= TrailPips / OneOverPoint && OrderStopLoss() < Bid - TrailPips / OneOverPoint - 0.0002)
                     if (OrderModifyCheck(OrderTicket(), OrderOpenPrice(), Bid - TrailPips / OneOverPoint + 0.0002, Bid - TrailPips / OneOverPoint + 0.0002 + 0.002)) {
                        if (CheckStopLoss_Takeprofit(OP_BUY, Bid - TrailPips / OneOverPoint + 0.0002, Bid - TrailPips / OneOverPoint + 0.0002 + 0.002))
                           ModifyOrder = OrderModify(OrderTicket(), OrderOpenPrice(), Bid - TrailPips / OneOverPoint + 0.0002, Bid - TrailPips / OneOverPoint + 0.0002 + 0.002, 0, Yellow);
                     }
            }
         }
      }
                  ...
                  if (OrderOpenPrice() - Ask >= TrailPips / OneOverPoint && OrderStopLoss() > Ask + TrailPips / OneOverPoint - 0.0002)
                     if (OrderModifyCheck(OrderTicket(), OrderOpenPrice(), Ask + TrailPips / OneOverPoint - 0.0002, Ask + TrailPips / OneOverPoint - 0.0002 - 0.002)) {
                        if (CheckStopLoss_Takeprofit(OP_SELL, Ask + TrailPips / OneOverPoint - 0.0002, Ask + TrailPips / OneOverPoint - 0.0002 - 0.002))
                           ModifyOrder = OrderModify(OrderTicket(), OrderOpenPrice(), Ask + TrailPips / OneOverPoint - 0.0002, Ask + TrailPips / OneOverPoint - 0.0002 - 0.002, 0, Yellow);
                     }
            }
         }
      }
      ...
      if (true) {
         ...
         if (True) {
            ...
            for (g_pos_424 = OrdersTotal() - 1; g_pos_424 >= 0; g_pos_424--) {
               SelectOrder = OrderSelect(g_pos_424, SELECT_BY_POS, MODE_TRADES);
                     ...
                     if (OrderSymbol() == Symbol() && OrderMagicNumber() == g_magic_248) {
                        if (OrderStopLoss() > 0.0) {
                           ...
                           if (OrderModifyCheck(OrderTicket(), g_price_296, OrderStopLoss(), g_price_252)) {
                              ...
                              ModifyOrder = OrderModify(OrderTicket(), g_price_296, 0, g_price_252, 0, Yellow);
                              ModifyOrder = OrderModify(OrderTicket(), g_price_296, OrderStopLoss(), g_price_252, 0, Yellow);
                           }
                        } else
                        //  
                        {
                           if (OrderModifyCheck(OrderTicket(), g_price_296, OrderOpenPrice() - Stoploss / OneOverPoint, g_price_252)) {
                              ...
                              if (CheckStopLoss_Takeprofit(OP_BUY, OrderOpenPrice() - Stoploss / OneOverPoint, g_price_252))
                                 if (CheckRange(OrderOpenPrice() - Stoploss / OneOverPoint, g_price_252))
                                    ModifyOrder = OrderModify(OrderTicket(), g_price_296, OrderOpenPrice() - Stoploss / OneOverPoint, g_price_252, 0, Yellow);
                           }
                        }
                     }
                     gi_440 = false;
         ...
         if (once) {
            Alert("Works only on EURUSD 1 Minute Chart but You are trying to use it on " + Symbol() + " " + Period());
            once = false;
         }
      }
      if (gi_164 == true) {
            ...
            if (gi_440) {
               if (gi_380 == true) {
                  for (g_pos_424 = OrdersTotal() - 1; g_pos_424 >= 0; g_pos_424--) {
                     ...
                     if (OrderSymbol() == Symbol() && OrderMagicNumber() == g_magic_248) {
                        if (OrderStopLoss() > 0.0)
                           if (OrderModifyCheck(OP_BUY, g_price_296, OrderStopLoss(), g_price_252)) { //Print(" Error 1 From C");
                              if (CheckStopLoss_Takeprofit(OP_BUY, OrderStopLoss(), g_price_252))
                                 ModifyOrder = OrderModify(OrderTicket(), g_price_296, OrderStopLoss(), g_price_252, 0, Yellow);
                           }
                        else {
                           if (OrderType() == OP_SELL)
                              if (OrderModifyCheck(OrderTicket(), g_price_296, OrderOpenPrice() + Stoploss / OneOverPoint, g_price_252)) { //Print(" Error 1 From D");
                                 if (CheckStopLoss_Takeprofit(OP_SELL, OrderOpenPrice() + Stoploss / OneOverPoint, g_price_252))
                                    ModifyOrder = OrderModify(OrderTicket(), g_price_296, OrderOpenPrice() + Stoploss / OneOverPoint, g_price_252, 0, Yellow);
                              }
                           else
                           if (OrderType() == OP_BUY)
                              if (OrderModifyCheck(OrderTicket(), g_price_296, OrderOpenPrice() - Stoploss / OneOverPoint, g_price_252)) { //Print(" Error 1 From E"); 
                                 ModifyOrder = OrderModify(OrderTicket(), g_price_296, OrderOpenPrice() - Stoploss / OneOverPoint, g_price_252, 0, Yellow);
                              }
                        }
                     }
...
int CountTrades() {
   ...
   for (int l_pos_4 = OrdersTotal() - 1; l_pos_4 >= 0; l_pos_4--) {
      SelectOrder = OrderSelect(l_pos_4, SELECT_BY_POS, MODE_TRADES);
      if (OrderSymbol() != Symbol() || OrderMagicNumber() != g_magic_248) continue;
      if (OrderSymbol() == Symbol() && OrderMagicNumber() == g_magic_248)
...
void ShowInfo() {
   ...
   Comment("",
      ...
      "\nEquity Minimal = ", DoubleToString(EquityMin, 2), 
      "\nEquity Maximal = ", DoubleToString(EquityMax, 2));
