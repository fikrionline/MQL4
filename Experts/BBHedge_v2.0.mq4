...
int TicketOrderSend, TicketOrderSelect, TicketOrderModify, TicketOrderClose, TicketOrderDelete, TotalOrderBuy, TotalOrderSell, LastTicket, LastTicketTemp, NumOfTradesSell, NumOfTradesBuy, MagicNumberBuy, MagicNumberSell, cnt, NewSignal = 0;
   ...
   if(RealStartBuy == FALSE) {
      ...
      if(BufferTotalOrderBuy < 1) {
         ...
         if((Hour() >= StartHour && Hour() < EndHour)) {
            //NewSignal = 1; //Remove this if use indicator
            if(NewSignal == 1) {
...
int GetSignal() {
   ...
   if(Ask < iBands(Symbol(), PERIOD_CURRENT, 20, 2, 0, PRICE_CLOSE, MODE_LOWER, 1)) {
      SignalResult = 1;
   }
   
   if(Bid > iBands(Symbol(), PERIOD_CURRENT, 20, 2, 0, PRICE_CLOSE, MODE_UPPER, 1)) {
      SignalResult = -1;
   }
