...
extern double EquityStopEA = 9000.0; //Stop EA when equity less than EquityStopEA 

extern string Version_3_9_6 = "EA Settings:";
extern string TradeComment = "Bismillahirrohmanirrohim";
// Enter a unique number to identify this EA
extern int EANumber = 5758;
...
extern int BreakEvenTrade = 5;
// Pips added to Break Even Point before BE closure
extern double BEPlusPips = 200;
...
extern string TP_SetArray = "250,500,1000";
...
string FileName, ID, StatFile, TheAccountEquity;
...
int deinit() {
   ...
   case REASON_CHARTCHANGE:
      if (CpT > 0)
         while (CpT > 0) CpT -= ExitTrades(P, displayColorLoss, "Blessing Removed");
...
int start() {
   ...
   TheAccountEquity = DoubleToString(AccountEquity(), 2);
   ...
   if (UseCloseOldest && CbT >= CloseTradesLevel && CbC < MaxCloseTrades) {
      if (!FirstRun && TPb > 0 && (ForceCloseOldest || (CbB > 0 && OPbO > TPb) || (CbS > 0 && OPbO < TPb))) {
         ...
         if (y == 1) {
            checkResult = OrderSelect(TbO, SELECT_BY_TICKET);
   ...
   if ((Testing && Visual) || !Testing) {
      if (displayOverlay) {
         ...
         DrawLabel("B3VPBal", TheAccountEquity, 167);
...
void LabelCreate() {
   if (displayOverlay && ((Testing && Visual) || !Testing)) {
      ...
      CreateLabel("B3LPBal", "Equity:", 0, 0, 0, 6);
