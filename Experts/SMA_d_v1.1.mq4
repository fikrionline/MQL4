...
extern double PnLStopAfterAdditionalOrder = 2;
         ...
         if(BuyOrSell == BuyOnly || BuyOrSell == BuySell) {
            ...
            if(GetSignal() == 1) {
               ...
               if(TotalOrderBuy <= 0) {
                  ...
                  TicketOrderSend = OrderSend(Symbol(), OP_BUY, StartingLot, Ask, SlipPage, StopLossBuy, 0, Symbol() + "_buy_1", MagicNumberBuy, 0, clrLime);
               ...
               if(TotalOrderBuy > 0) {
                  ...
                  if ((LastBuyPrice - Ask) >= (PipStep * Point) && (Ask - LastBuyPriceSL) > (PipStep * Point)) {
                     ...
                     TicketOrderSend = OrderSend(Symbol(), OP_BUY, AdditionalLot, Ask, SlipPage, LastBuyPriceSL, 0, Symbol() + "_buy_" + NumOfTradesBuy, MagicNumberBuy, 0, clrLime);
                     if (TicketOrderSend < 0) {
                        Print("Error: ", GetLastError());
                     }
                  }
         ...
         if(BuyOrSell == SellOnly || BuyOrSell == BuySell) {
            ...
            if(GetSignal() == -1) {
               ...
               if(TotalOrderSell <= 0) {
                  ...
                  TicketOrderSend = OrderSend(Symbol(), OP_SELL, StartingLot, Bid, SlipPage, StopLossSell, 0, Symbol() + "_sell_1", MagicNumberSell, 0, clrRed);
               }
               ...
               if(TotalOrderSell > 0) {
                  ...
                  if ((Bid - LastSellPrice) >= (PipStep * Point) && (LastSellPriceSL - Bid) > (PipStep * Point)) {
                     ...
                     TicketOrderSend = OrderSend(Symbol(), OP_SELL, AdditionalLot, Bid, SlipPage, LastSellPriceSL, 0, Symbol() + "_sell_" + NumOfTradesSell, MagicNumberSell, 0, clrLime);
                     if (TicketOrderSend < 0) {
                        Print("Error: ", GetLastError());
                     }
                  }
